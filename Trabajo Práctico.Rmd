...
*El Trabajo práctico Nº 2 arranca en la línea 293*
...
Primero, creamos una caja de coordenadas de la CABA para decirle a R que nos traiga la info de allí.

```{r}
bbox_CABA <- getbb("Ciudad Autónoma de Buenos Aires, Argentina")
```

*Nos fijamos que las coordenadas extraídas sena las precisas*
...
Segundo, queremos crear un dataset con las calles de CABA. Para ello:
...
Por último, coloreamos nuestras calles de acuerdo con el tipo.
...
Al ver el gráfico anterior, podemos observar que hay algo que no anda bien. Nuestro dataset de estacionamientos contiene 87 registros, aunque solo aparecen geolocalizados unos pocos puntos. Ello puede deverse a que la posición de los mismos esta mal cargada.
...
Como podemos ver en el mapa anterior, la mayor concentración de estacionamientos se da en el corredor que se extiende desde el centro de la CABA hacia el norte. No obstante, es importante reconocer que los enstacionamientos se muesntran distribudios a lo largo y ancho de la ciudad.
...
Ahora, crucemos nuestro dataset de estacionamientos con el de barrios para conocer su distribución por cada uno de estos últimos.
...
Estas diferencias pueden deverse a varios factores. Sin embargo, es importante reconocer que, mientras nuestro GEOJSON original nos daba solo la ubicación de los estacionamientos concesionados, el nuevi dataset con datos de OSM ofrece información de todos los tipos de estacionamientos existentes en la CABA.
...
ggplot()+
  geom_sf(data = microcentro, fill="grey96")+
  geom_sf(data = espacioverde, fill="springgreen2")+
  geom_sf(data = estacionamientos, aes(color="Estacionamientos GCBA"), size=2)+
  geom_sf(data=parking_microcentro, aes(color="Estacionamientos OSM"), size=2)+
