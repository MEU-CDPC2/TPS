...
```{r include=FALSE}
sf::sf_use_s2(FALSE) #Qué es esto, Luis?
library(tidyverse)
library(sf)
library(ggmap)
library(scales)
...
```{r}
viajesbicis_microcentro <- read.csv("https://raw.githubusercontent.com/MEU-CDPC2/TPS/main/Datasets/bicis_microcentro_origen_destino.csv", stringsAsFactors = TRUE, encoding = "UTF-8") #Solo 99 viajes hubo en todo 2020 en el microcentro???
```
...
ggmap(mapamicrocentro)+
  ...
  geom_sf(data = ecobicis, aes(fill="Ecobicis"), color="orange", size=4, inherit.aes = FALSE)+
  labs(title="Recorridos más realizados en Bicicleta en el Microcento",
       subtitle="Viajes lineales",
       fill="",
       color="Duración \n (Minutos)",
       caption="Fuente: BA Data")+
