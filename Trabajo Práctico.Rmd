...
Habiendo identificado las estaciones más frecuentes, el uso anual y el comportamiento de los usuarios a lo largo del día, nos preguntamos:
    ...
    ¿Cómo se movió la gente en bici dentro del microcentro?
...
a. *Elegir un dataset que contenga viajes origen-destino de la Ciudad con la que están trabajando.*
...
```{r}
paste(round(mean(bicis_microcentro$duracion_minutos),2), " minutos es el promedio de duracion de viajes. Sin embargo, la mediana está en ", round(median(bicis_microcentro$duracion_minutos),2), " minutos, por lo que, al igual que en el análisis correspondiente al TP3, reconocemos un sesgo positivo (cola hacia la derecha)")
```
