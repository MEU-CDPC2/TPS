...
```{r}
parking_BA_OSM <- opq ("Buenos Aires, Argentina") %>% 
  add_osm_feature(key="amenity", value="parking")
parking_BA_OSM <- osmdata_sf(parking_BA_OSM) 
parking_BA_OSM <- parking_BA_OSM$osm_points
```

```{r}
parking_BA_OSM <- parking_BA_OSM %>% select(osm_id, parking, geometry)
```

```{r}
parking_BA_OSM <- st_intersection(parking_BA_OSM, barrios, crs=(barrios))

parking_BA_OSM_barrio <- parking_BA_OSM %>% 
  group_by(barrio) %>% 
  summarise(cantidad_OSM=(n())) %>% 
  as.data.frame() %>% 
  st_set_geometry(NULL) %>% 
```
```{r fig.height=10}
ggplot()+
  geom_bar(data=parking_BA_OSM_barrio %>% filter(cantidad_OSM!=0), aes(y=reorder(barrio, -cantidad_OSM), weight=cantidad_OSM, fill=barrio), show.legend = FALSE)+
  scale_fill_viridis_d()+
  labs(x="Cantidad de estacionamientos", 
       y="Barrio",
       title="¿Cuántos estacionamientos por barrio existen según OSM?", 
       caption="Fuente: OSM")+
...
```{r}
parking_BA <- left_join(barrios, parking_BA_OSM_barrio, by="barrio")
```
...
ggplot()+
  geom_sf(data=barrios, fill="white")+
  geom_sf(data=parking_BA %>% filter(cantidad_OSM!=0), aes(fill=cantidad_OSM), alpha=.5, show.legend = FALSE)+
  geom_sf_text(data=parking_BA, aes(label=cantidad_OSM), size=4, fontface = "bold")+
  scale_fill_viridis_c()+
...
parking_OSM_GCBA <- left_join(parking_BA_OSM_barrio, estacionamientos_barrio, by="barrio") %>% 
  rename("cantidad_GCBA"="cantidad") %>% 
  group_by(barrio, cantidad_OSM, cantidad_GCBA) %>%
  summarise(diferencia=cantidad_OSM-cantidad_GCBA)
```
```{r}
...
```{r fig.height=10}
ggplot()+
  geom_bar(data=parking_OSM_GCBA, aes(y=reorder(barrio, -cantidad_OSM), weight=cantidad_OSM, fill=barrio), show.legend = FALSE)+
  geom_bar(data=parking_OSM_GCBA, aes(y=reorder(barrio, -cantidad_OSM), weight=cantidad_GCBA, fill=barrio), fill="red", show.legend = FALSE)+
  scale_fill_viridis_d()+
...
```{r}
parking_microcentro <- parking_BA_OSM %>% 
