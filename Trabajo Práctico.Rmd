...
### <a id="tp_5"></a>**Trabajo Práctico N°5**


Habiendo identificado las estaciones más frecuentes, el uso anual y el comportamiento de los usuarios a lo largo del día, nos preguntamos
  
    ¿Cómo se mueve la gente en bici dentro del microcentro?
    ¿Existe alguna correlación espacial entre las estaciones origen-destino?
    ¿Es frecuente la presencia de viajes circulares?

**Consignas**
  
  
a. *Elegir n dataset que contenga viajes origen-destino de la Ciudad con la que están trabajando.*

  
Para realizar este ejercicio hemos filtrado previamente el dataset original de recorridos de bicis públicas del GCBA. Nos quedamos sólo con los recorridos que iniciaron y finalizaron dentro del microcentro, para entender el comportamiento limitado a nuestra área de estudio.

Una vez identificados los IDs de las estaciones involucradas, procedidos a filtrar los recorridos:
   
    
*filter (id_estacion_origen==c(51, 7, 83, 191, 171, 24, 8, 163, 144, 183, 112, 37, 60, 132))*
*filter (id_estacion_origen==c(51, 7, 83, 191, 171, 24, 8, 163, 144, 183, 112, 37, 60, 132))*


Omitimos este paso debido al gran peso del archivo base.  
...
```{r}
bicis_microcentro <- read.csv("Datasets/bicis_microcentro_origen_destino.csv")
estaciones_bicis <- read.csv("Datasets/estaciones_bici.csv")
```

```{r}
head(bicis_microcentro)
```


Notamos que sólo existen 99 recorridos inicados y finalizados dentro del microcentro
  
A modo de comparación el análisis previo y para entender el cambio en los patrones de uso:
  
**¿Cúantos minutos duran los viajes en bicicleta iniciados y finalizados dentro del microcentro en promedio?**

```{r}
bicis_microcentro <- bicis_microcentro %>%
  mutate(duracion_minutos = round(as.numeric(as.duration(duracion_recorrido), "minutes"),2)) 
```

```{r}
paste(round(mean(bicis_microcentro$duracion_minutos),2), " minutos es el promedio de duracion de viajes. Sin embargo, la mediana está en ", round(median(bicis_microcentro$duracion_minutos),2), " minutos, por lo que, a al igual que en el análisis correspondiente al TP3, reconocemos un sesgo positivo (cola hacia la derecha)")
```

Al limitar el área de origen y destino notamos una considerable reducción del tiempo promedio de los viajes.  

```{r}
paste("La reducción en el tiempo promedio es de un ",  round(((mean(bicis_2020$duracion_minutos)/mean(bicis_microcentro$duracion_minutos)-1)*100),2), "%")
```

**¿Hay viajes circulares?**

```{r}
viajes <- bicis_microcentro %>% 
...
A simple vista se evidencia que sí hay presencia de viajes circulares y casualmente son los que más mayor frecuencia acumulan, principalmente en las siguientes estaciones:

| Origen-Destino | Nombre de la estación |
| ----------- | ----------- |
| 8 | Congreso |
| 144 | Pueyrredón |
| 183 | Virrey Cevallos |
...
Lo miramos más de cerca: 

```{r}
viajes_ciculares<- viajes[which(viajes$id_estacion_origen == viajes$id_estacion_destino), ]

head(arrange(viajes_ciculares, desc(cant_viajes)))
```


```{r}
ggplot(viajes_ciculares, aes(y=reorder(nombre_estacion_origen, -cant_viajes), x=cant_viajes)) +
  geom_bar(stat="identity", fill="orange", alpha=.5, color="black")+
  labs(title = "Viajes circulares en bici",
       subtitle = "Estaciones de bicis del microcentro porteño",
...
```{r}
viajes_ciculares<- st_as_sf(viajes_ciculares, coords = c("long_estacion_origen", "lat_estacion_origen"), crs = 4326)

ggplot()+
  geom_sf(data = microcentro, fill="grey96")+
  geom_sf(data=viajes_ciculares, aes(size=cant_viajes),color="orange", alpha=0.8)+
...
b. *Calcular los 10 recorridos más realizados, describir los resultados obtenidos y hacer un mapa con los ruteos.*
...
Las 3 estaciones con más viajes coinciden con las estaciones con más viajes circulares
Procedemos a agregarle la información al dataset que generamos: 
...
```{r}
ruteo_top_10 <- list(top_10$nombre_estacion_origen, top_10$long_estacion_origen, top_10$lat_estacion_origen,
                   top_10$nombre_estacion_destino, top_10$long_estacion_destino, top_10$lat_estacion_destino)
```
...
```{r}
summary(ruteo_top_10) 
```

```{r}
ggplot()+
  geom_sf(data = microcentro, fill="grey96")+
  geom_sf(data=ruteo_top_10, aes(color=duration), size=2, inherit.aes = FALSE)+
...
#ESTE GRAFICO NO ESTA SALIENDO BIEN +
#ESTARIA BUENO USAR GGMAP Y LEAFLET
```
