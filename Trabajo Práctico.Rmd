...
COMENTARIOS PAULA A INCORPORAR: Creo que sería interesante sumar abril en los gráficos, ya que se vería vacío.En el histograma es bastante sencillo: pueden configurar una escala de fecha con scale_x_date() En el caso del mapa, deberían primero manipular (tidyverse) los datos, generando un dataset con cantidad de viajes x mes x estación. Luego debería agregar estaciones con 0 viajes en abril
...
```{r}
tweets_verdes <- search_tweets(q = "espacio verde",
...
Y, aprovechando que recientemente fue comunicado un plan para revitalizar el microcentro, buscaremos aquellos tweets que contengan la palabra "microcentro".

```{r}
tweets_microcentro <- search_tweets(q = "microcentro",
                                    geocode = "-34.606022,-58.433788,6.5mi",
                                    include_rts = FALSE,
                                    n = 100000,
                                    retryonratelimit = TRUE)
...
### MIRA LUIS QUE GUARDE LOS DATASETS COMO ARCHIVOS RDS PARA QUE NO SE NOS VAYAN. SI QUERES CAMBIAR LAS PALABRAS O USAR OTROS DATASETS, ESTA TODO REGIO.
...
```{r}
ggplot(tweets) +
  geom_histogram(aes(x = followers_count), fill="lightblue")+
...
A simple vista, la gran mayoría de los twitteros poseen solo un puñado de seguidores. Veamos con mayor profundidad.
...
Al observar nuestros datos, podemos ver que los tweets seleccionados poseen un volumen relativamente bajo de retweets. Curiosamente, los tweets más populares de nuestras muestra no fueron escritos por ninguna de las 5 cuentas más populares antes mencionadas.
...
Ahora veremos a que hora se twitea con más intensidad. 

ACÁ Habría que ver que onda lo de la Time Zone.

```{r}
tweets %>%
  mutate(created_at = ymd_hms(created_at)) %>% 
  count(hora = hour(created_at)) %>% 
ggplot() +
  geom_col(aes(x = hora, y = n), fill="lightblue")+
...
Podemos ver que la gente twitea más a medida que avanza el día, y llega a un pico de actividad antes de la medianoche.
...
Creo que habría que concluir qeu este es un tema que se pasan por las bolas en las redes.