...
```{r message=FALSE, warning=FALSE}
paste("La reducción en el tiempo promedio es de un ",  round(((mean(bicis_2020$duracion_minutos)/mean(viajesbicis_microcentro$duracion_minutos)-1)*100),2), "%")
```
...
ruteo_top_10 <- ruteo_top_10 %>% 
  st_jitter(amount=.00035) #separamos levemente los recorridos para evitar superposición y visualizarlos a todos

ggmap(mapamicrocentro)+
  geom_sf(data=ruteo_top_10_2, aes(color=duration), size=1.5, inherit.aes = FALSE)+
...
Además, notamos que las estaciones con mayor frecuencia de viajes circulares se ubican geográficamente en la misma traza. 
El único ruteo que se superpone en nuestro top 10 de viajes más frecuentes, es el correspondiente al tramo de las estaciones Congreso-Paraná, que registra viajes en ambos sentidos.    
