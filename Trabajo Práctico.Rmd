...
Previo a realizar cualquier intervención sobre el Microcentro, debemos indagar primero sobre la opinión que tienen porteños de la dotación actual de espacios públicos verdes en la ciudad. Por este motivo, el siguiente trabajo se propone realizar un análisis de redes a fin de identificar las principales nociones de los habitantes de la ciudad en lo respectivo a los espacios verdes del distrito, y, particularmente, los de su área central. 

*CONSIGNAS*

1. *Descargar tweets que se originen en los alrededores de la Ciudad con la que están trabajando.*

Nuestro primer paso será descargar los tweets emitidos en la CABA que contengan las palabras "espacio verde". Para delimitar el área, seleccionamos las coordenadas del parque Centenario y establecemos un radio de 6,5 millas, la distancia que existe en promedio al borde del distrito.

```{r}
tweets_verdes<- search_tweets(q = "espacio verde",
              geocode = "-34.60602274602465, -58.433788440208026,6.5mi",
              include_rts = FALSE,
              n = 100000,
...
Para quienes quieran guardar los datos que descarguen pueden usar los siguientes chunk dependiendo del formato que quieran:

```{r}
saveRDS(midataset, file = "midataset.rds")
```

```{r}
write_as_csv(midataset, "midataset.csv", prepend_ids = TRUE, na = "", fileEncoding = "UTF-8")
```